...
# NormalisedGenerality <- function(M){
#   return(TrophicGenerality(M)/(sum(M)/dim(M)[2]));
# }
# 
# NormalisedVulnerability <- function(M){
...
MeanFoodChainLength_Old <- function(M){
...
FindCycles = function(g) {
  ...
  for(v1 in V(g)) {
    ...
    GoodNeighbors = neighbors(g, v1, mode="out") ## outgoing links only (v1 is eaten by neighbors)
    # GoodNeighbors = GoodNeighbors[GoodNeighbors > v1]
    
    for(v2 in GoodNeighbors) { 
      TempCyc = lapply(all_simple_paths(g, v2,v1, mode="out"), function(p) c(v1,p)) ## looking for prey of v1 that v1 also eats
...
NewMeanFoodChainLengthSlow <- function(g){
  
  Cycles <- FindCycles(g)
  links = unique(unlist(lapply(Cycles, function(x) paste(names(x)[2:3], collapse = '|'))))
  g = delete.edges(g,links)

  # g = topo_sort(g, mode = 'out') # nodes with no incoming edges go first
  
  TL = data.frame(code = attr(V(g),'names'), TL = 0, count = 0)
  
  basal <- which(degree(g, mode='in') == 0)
  top <- which(degree(g, mode='out') == 0)
  if (length(top) == 0){
      int <- which(degree(g, mode='out') != 0 & degree(g, mode='in') != 0)
      top = int ## when there are no top predators, the int become top
  }
  ...
  for(b in basal){
    for(t in top){
      ...
      all_paths <- all_simple_paths(g,b,t)
      
      if(length(all_paths) > 0){
        lengths <- append(lengths, unlist(lapply(all_paths, length)))
        
        ## function for TL
        TL[,'TL'] <- TL[,'TL'] + Reduce('+',lapply(all_paths, function(x) match(TL[,'code'],attr(x,'names'), nomatch = 0)))
        TL[,'count'] <- TL[,'count'] + Reduce('+',lapply(all_paths, function(x) match(TL[,'code'],attr(x,'names'), nomatch = 0)/
                                                           match(TL[,'code'],attr(x,'names'), nomatch = 1000)))
      }
    }
  }
  
  TL$mean.TL = TL[,'TL']/TL[,'count']
  
  # lengths <- lengths[-which(lengths == 0)]
  return(c(mean(lengths), TL))  
}



MeanFoodChainLength <- function(g){
  ...
  ## 2. topological sort
  order = topological.sort(g, mode = 'out') # nodes with no incoming edges go first
  g.sort = permute(g, match(V(g),order))
  
  adj = as.matrix(as_adjacency_matrix(g.sort))
  community = cheddar::Community(nodes=data.frame(node = colnames(adj)), trophic.links=PredationMatrixToLinks(adj), properties=list(title='Community'))
  
  ### TL from cheddar
  TL = ChainAveragedTrophicLevel(community)
  ## Mean food chain length from cheddar
  chain.stats <- TrophicChainsStats(community)
  fcl <- (chain.stats$chain.lengths + 1)
  
  mfcl = (sum(fcl)/length(fcl))
  
  return(list(mfcl = mfcl, ChainAveragedTrophicLevel = TL))  
}
...
TrophicPosition <- function(matrix){
  M <- t(matrix)
  node <- 1:dim(M)[1];
  for(n in 1:length(node)){
    node[n] <- paste(node[n],'-');
  ...
  community <- Community(nodes=data.frame(node=node), trophic.links=PredationMatrixToLinks(pm), properties=list(title='Community'));
  a <- data.frame(species = row.names(m_uk), trophic_position = TrophicHeight(community))
  
  return(a)
}

TrophicPositionsNew <- function(adj_matrix){ 
  require(cheddar)
  M <- t(adj_matrix)
  node <- 1:dim(M)[1];
  for(n in 1:length(node)){
  ...
  community <- Community(nodes=data.frame(node=node), trophic.links=PredationMatrixToLinks(pm), properties=list(title='Community'));
  a <- data.frame(species = row.names(adj_matrix), trophic_position = ChainAveragedTrophicLevel(community))
