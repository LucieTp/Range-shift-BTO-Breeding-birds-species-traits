...
species_traits <- read.csv('data/SpecTrait_062023_157sp.csv', stringsAsFactors = FALSE, row.names = 1)

#### reading the range shift data 
range_shifts <- read.csv('data/df_rangeshift_062023.Mainland1.csv', stringsAsFactors = FALSE, row.names = 1)

species_traits <- merge(species_traits, range_shifts, by = 'speccode')
...
cells_info <- read.dbf('data/mask10k/reference_grid_10km.img.vat.dbf')
...
species_traits$scientific_name_web <- stringr::str_replace(species_traits$scientific_name_ET,' ', '_')
species_traits[which(is.na(match(species_traits$scientific_name_web, row.names(m_uk)))),]$scientific_name_web <- c("Branta_canadensis","Saxicola_torquata")
# can't find canada goose in the metaweb Branta canadensis

library(stringr)
# Branta canadensis is not in the metaweb
row.names(m)[which(str_detect(row.names(m),"Branta"))] 
