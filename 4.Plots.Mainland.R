...
length(poleward.leading) + length(southward.leading) + length(stable.leading) == 135
...
wilcox.test(shift_max20_P.1.3_dist_km ~ PassNonPass, data = species_traits) 

mean(species_traits$shift_max20_P.1.3_dist_km[which(species_traits$distrib.core == 'north')])
mean(species_traits$shift_max20_P.1.3_dist_km[which(species_traits$distrib.core == 'south')])

mean(species_traits$shift_min20_P.1.3_dist_km[which(species_traits$distrib.core == 'north')])
sd(species_traits$shift_min20_P.1.3_dist_km[which(species_traits$distrib.core == 'north')])

mean(species_traits$shift_min20_P.1.3_dist_km[which(species_traits$distrib.core == 'south')])
...
Northern.dist = ggplot(data = sf_UK) + geom_sf(colour = "lightgrey") + geom_point(data = loc, aes(x = long, y = lat, colour = period), size = 2, shape = 15) +
  ...
  geom_hline(yintercept = min(BTO_distrib$lat), linetype="dashed", size = 2) + geom_hline(yintercept = 58.7, linetype="dashed", size = 2) + scale_colour_manual(values = c("black", "gray57","#CC6677","#88CCEE")) +
...
##################
## plots for subplots (transparent bg)
